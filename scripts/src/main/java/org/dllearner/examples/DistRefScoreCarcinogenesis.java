...
import org.dllearner.algorithms.distributed.amqp.DistScoreAndRefinementCELOEAMQP;
...
public class DistRefScoreCarcinogenesis {
    private static final Logger logger = Logger.getLogger(DistRefScoreCarcinogenesis.class);
...
//        op.setDataPropertyHierarchy(baseReasoner.getDatatypePropertyHierarchy());
        ...
        DistScoreAndRefinementCELOEAMQP celoe = new DistScoreAndRefinementCELOEAMQP(lp, rc);
//        DistScoreAndRefinementCELOEAMQP celoe =
//                new DistScoreAndRefinementCELOEAMQP(lp, baseReasoner);
...
//        celoe.updateAMQPSettings("/tests/dllearner_amqp/dist_score_and_refinement/amqp.properties");
        ...
        if (args.length > 3) {
            double workerRefinementScoreRatio = Double.parseDouble(args[3]);
            celoe.setRefinementScoreRatio(workerRefinementScoreRatio);
        }
        if (args.length > 4) {
            int execTime = Integer.parseInt(args[4]);
            celoe.setMaxExecutionTimeInSeconds(execTime);
        } else {
