...
\begin{description}
 \item[OWL Class Expression Learner (OCEL)] \todo{update} The previous algorithm has been extended to make more sophisticated use of background knowledge~\cite{mlj} and therefore run more efficiently on many problems. It also supports double datatypes and hasValue restrictions (which again can be turned on or off as desired). It also includes explicit noise handling through the \verb|noisePercentage| option. This is currently the default and recommend algorithm for learning from positive and negative examples. More than 30 options can be set to control its behaviour. However, apart from the target language the most important setting is noise, which should be optimised for the given problem.
 ...
 \item[Inductive Statistical Learning of Expressions (ISLE)] \todo{write}
\end{description}
...
\begin{itemize}
 ...
 \item Latest Release: \url{https://github.com/AKSW/DL-Learner/releases/tag/v1.2}
\end{itemize}
