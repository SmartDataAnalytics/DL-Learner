...
public class ParCELExFortifiedCrossValidation extends CrossValidation {
	...
	public ParCELExFortifiedCrossValidation(AbstractClassExpressionLearningProblem la, PosNegLP lp, AbstractReasonerComponent rs,
	 ...
	 */
	public ParCELExFortifiedCrossValidation(AbstractClassExpressionLearningProblem la, ParCELPosNegLP lp, AbstractReasonerComponent rs,
											int folds, boolean leaveOneOut, int noOfRuns) {
				...
				if (tmp3.size() > 0) {
					...
					for (ParCELExtraNode pdef : reducedPartialDefinitions) {
						...
						if (coveredNeg.size() > 0) {
							...
							TreeSet<OWLIndividual> tempCoveredNeg = new TreeSet<>(new URIComparator());
							...
							for (ParCELExtraNode cpdef : counterPartialDefinitions) {
								Set<OWLIndividual> cpdefNegCovered = new HashSet<>(rs.hasType(cpdef.getDescription().getChild(0), negTestSet));
								logger.info("     - cpdef: " + cpdef.getDescription().getChild(0).toKBSyntaxString(baseURI, prefixes) + 
								...
								if (tempCoveredNeg.removeAll(cpdefNegCovered)) {
									...
									Set<OWLIndividual> cpdefPosCovered = new HashSet<>(rs.hasType(cpdef.getDescription().getChild(0), testSetsPos.get(currFold)));
					...
					for (ParCELExtraTestingNode n : fortifiedLevel1) {
						if (tmp3.removeAll(n.getCoveredNegativeExamplestestSet())) {
							...
							addedLength += n.getExtraNode().getDescription().getLength(); 
	...
	*/
	...
	class URIComparator implements Comparator<OWLIndividual> {

		@Override
		public int compare(OWLIndividual o1, Individual o2) {
