...
public class CELOEFortifiedCrossValidation3PhasesFair extends CrossValidation {
	...
	protected Stat noOfCounterPartialDefinitionsStat;
	...
	*/
	 ...
	 * @param leaveOneOut
	 * @param noOfRuns
	 * @param fortificationNoise
	 * @param fortificationTimeout
	...
	public CELOEFortifiedCrossValidation3PhasesFair(AbstractCELA la, PosNegLP lp, AbstractReasonerComponent rs, int folds,
		...
		Set<OWLIndividual> posExamples = ((PosNegLP) lp).getPositiveExamples();
		List<OWLIndividual> posExamplesList = new LinkedList<>(posExamples);
		//Collections.shuffle(posExamplesList, new Random(1));
		Set<OWLIndividual> negExamples = ((PosNegLP) lp).getNegativeExamples();
		...
		for (int kk = 0; kk < noOfRuns; kk++) {
			...
			noOfCounterPartialDefinitionsStat = new Stat();
			...
			for (int currFold = 0; currFold < folds; currFold++) {
				...
				noOfCounterPartialDefinitionsStat.addNumber(counterPartialDefinitions.size());
				...
				outputWriter("Noise: " + orgNoise + "%, timeout=" + orgTimeout);
...
//				totalNumberOfDescriptions.addNumber(noOfDescriptionGeneratedPdef);
				...
				for (CELOEPartial.PartialDefinition cpdef : counterPartialDefinitions) {
					...
					//BLIND Fortification
					//--------------------
					//get set of covered pos. and neg. examples in test set
...
//				outputWriter("  total number of descriptions: " + noOfDescriptionGeneratedPdef);
				...
				outputWriter("  no of counter partial definitions: " + statOutput(df, noOfCounterPartialDefinitionsStat, ""));
				outputWriter("  avg counter partial definition length: " + statOutput(df, avgCPDefLengthStat, ""));
			...
			for (int i=0; i< noOfStrategies; i++) {
				...
				outputWriter("  multi-step fortified accuracy by " + FortificationUtils.strategyNames[i] + "(cpdef:" + noOfCpdefUsedMultiStepFortStat[i] + "):");
