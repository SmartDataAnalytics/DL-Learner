	...
	public ParCELExFortifiedCrossValidation3Phases(AbstractCELA la, ParCELPosNegLP lp, AbstractReasonerComponent rs,
		...
		for (int kk=0; kk < noOfRuns; kk++) {
			...
			for(int currFold=0; (currFold<folds); currFold++) {
				...
				Set<OWLIndividual> upTest = Helper.difference(curFoldPosTestSet, cpTest);	//false negative (pos as neg)
				...
				for (CELOE.PartialDefinition negCpdef : counterPartialDefinitions) {
					...
					OWLClassExpression cpdef = negCpdef.getDescription().getChild(0);
					...
					Set<OWLIndividual> commonCp = new HashSet<Individual>();
					Set<OWLIndividual> commonCn = new HashSet<Individual>();
				...
				*/
				...
				if (cnTest.size() > 0) {
					...
					for (CELOE.PartialDefinition negCpdef : counterPartialDefinitions) {
						...
						Set<OWLIndividual> desCoveredNeg = new HashSet<Individual>(rs.hasType(cpdef, curFoldNegTestSet));
						...
						if (tempCoveredNeg.removeAll(desCoveredNeg)) {
							...
							Set<OWLIndividual> desCoveredPos = new HashSet<Individual>(rs.hasType(cpdef, curFoldPosTestSet));
