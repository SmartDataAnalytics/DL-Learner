    ...
    private Query convertCondition(DRS_Condition condition, Query query) {
        if (condition.isComplexCondition()) {
            ...
            if (predicate.equals("count")) {
            	// COUNT(?x) AS ?c
                query.addSelTerm(new SPARQL_Term(simple.getArguments().get(0).getValue(), SPARQL_Aggregate.COUNT, true, new SPARQL_Term(simple.getArguments().get(1).getValue(),true)));
                return query;
