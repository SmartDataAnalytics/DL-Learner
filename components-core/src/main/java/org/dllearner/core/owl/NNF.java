...
/**
 * 
 * A modified version of the NNF computation contained in the OWL API. The
 * main difference is in the usage of DL-Learner version of intersection and
 * union which allow for redundant usage of sub expression, e.g. (A and A)
 * 
...
public class NNF implements OWLClassExpressionVisitorEx<OWLClassExpression>,
    ...
    public OWLClassExpression visit(OWLObjectIntersectionOf desc) {
        ...
        if (negated) {
            return new OWLObjectUnionOfImplExt(ops);
        } else {
            return new OWLObjectIntersectionOfImplExt(ops);
        }
    }
