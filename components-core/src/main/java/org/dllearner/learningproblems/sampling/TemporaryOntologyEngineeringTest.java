...
import org.dllearner.learningproblems.AccMethodPredAccApprox;
 ...
 * 
 */
public class TemporaryOntologyEngineeringTest {
	...
	public TemporaryOntologyEngineeringTest(File ontFile, String outFile) throws OWLOntologyCreationException, IOException, ComponentInitException {
		  ...
		  if(!resultoutput.exists())
	         {
	           ...
	           BufferedWriter output = new BufferedWriter(new FileWriter(resultoutput));
...
//			run(rc);
			...
			Logger.getRootLogger().info("=== Step 1 ===");
...
//			run(rc);
			...
			Logger.getRootLogger().info("=== Step 2 ===");
...
//					op.setUseDataHasValueConstructor(true);
					...
					Logger.getRootLogger().info("CELOE terminated.");
...
//										+ "\") if any of the suggestions is appropriate (if several are possible choose the lowest number). Type \"n\" if there is no appropriate suggestion for this class in your opinion. Type \"m\" if there is an appropriate suggestion in your opinion, but the algorithm did not suggest it.");
		...
		// print summary
               ...
               FileOutputStream fos = new FileOutputStream(resultoutput, true);
        ...
        System.out.println("\n\n======= Stats New");
       	System.out.println("ClosedWorldReasoner : " + useFastInstanceChecker
       				+ "\nUse approximations : " + useApproximations + "\nuseFMeasure : "+ useFMeasure );
...
//				try {
					...
					new TemporaryOntologyEngineeringTest(ontFile, outFile);
//				} catch (Exception e) {
//					e.printStackTrace();
//				}
			}
