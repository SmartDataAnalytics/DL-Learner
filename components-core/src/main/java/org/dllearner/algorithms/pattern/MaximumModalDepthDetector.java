 ...
 *
 */
public class MaximumModalDepthDetector implements OWLAxiomVisitor, OWLClassExpressionVisitor{
	
	int maxModalDepth;
	
	public static int getMaxModalDepth(OWLAxiom axiom){
		MaximumModalDepthDetector depthDetector = new MaximumModalDepthDetector();
		axiom.accept(depthDetector);
		return depthDetector.maxModalDepth;
	}
	...
	public void visit(OWLSubClassOfAxiom axiom) {
		...
		int tmp = maxModalDepth;
		maxModalDepth = 1;
		OWLClassExpression superClass = axiom.getSuperClass();
		superClass.accept(this);
		maxModalDepth = Math.max(tmp, maxModalDepth);
	}
