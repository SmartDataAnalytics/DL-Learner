...
public class SimpleSubclassLearner extends AbstractAxiomLearningAlgorithm implements ClassExpressionLearningAlgorithm {
	...
	public List<EvaluatedAxiom> getCurrentlyBestEvaluatedAxioms(int nrOfAxioms) {
		...
		for(EvaluatedDescription ed : getCurrentlyBestEvaluatedDescriptions(nrOfAxioms)){
			currentlyBestAxioms.add(new EvaluatedAxiom(df.getOWLSubClassOfAxiom(classToDescribe, ed.getDescription()), new AxiomScore(ed.getAccuracy())));
		}
			...
			for(OWLClassExpression assertedSup : existingSuperClasses){
				if(reasoner.isPrepared()){
					if(reasoner.getClassHierarchy().contains(assertedSup)){
						for(OWLClassExpression inferredSup : reasoner.getClassHierarchy().getSuperClasses(assertedSup, false)){
	...
	private void runSingleQueryMode(){
		...
		if(total > 0){
			String query = String.format("SELECT ?type (COUNT(DISTINCT ?s) AS ?cnt) WHERE {?s a <%s>. ?s a ?type} GROUP BY ?type ORDER BY DESC(?cnt)", classToDescribe.toStringID());
	...
	private boolean addIndividualsWithTypes(Map<OWLIndividual, SortedSet<OWLClassExpression>> ind2Types, int limit, int offset){
		boolean notEmpty = false;
		String query;
		if(ks.supportsSPARQL_1_1()){
			query = String.format("PREFIX owl: <http://www.w3.org/2002/07/owl#> SELECT DISTINCT ?ind ?type WHERE {?ind a ?type.?type a owl:Class. {SELECT ?ind {?ind a <%s>} LIMIT %d OFFSET %d}}", classToDescribe.toStringID(), limit, offset);
		} else {
			query = String.format("SELECT DISTINCT ?ind ?type WHERE {?ind a <%s>. ?ind a ?type} LIMIT %d OFFSET %d", classToDescribe.toStringID(), limit, offset);
		}
		ResultSet rs = executeSelectQuery(query);
		OWLIndividual ind;
