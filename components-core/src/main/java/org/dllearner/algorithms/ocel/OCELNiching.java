...
public class OCELNiching extends OCEL {
	...
	public OCELNiching(PosNegLP learningProblem, AbstractReasonerComponent reasoningService) {
		super(learningProblem, reasoningService);
	}

	public OCELNiching(PosOnlyLP learningProblem, AbstractReasonerComponent reasoningService) {
		super(learningProblem, reasoningService);
	}
	...
	public void init() throws ComponentInitException {
		...
		if (!logLevel.equals(CommonConfigOptions.logLevelDefault))
			org.apache.log4j.Logger.getLogger(OCELNiching.class).setLevel(Level.toLevel(logLevel, Level.toLevel(CommonConfigOptions.logLevelDefault)));
	...
	public void start() {
		...
		Monitor totalLearningTime = JamonMonitorLogger.getTimeMonitor(OCELNiching.class, "totalLearningTime").start();
		...
		logger.info(
				"starting top down refinement with: " +
						renderer.render(startNode.getConcept()) + " (" +
						df.format(100 * startNode.getAccuracy()) +
						"% accuracy)");
		...
		while (!isTerminationCriteriaReached()) {
			...
			Iterator<ExampleBasedNode> it = searchTree.descendingIterator();
...
//                    computeSimilarityPenalty(node, searchTree.best());
	...
	protected double computeSimilarityPenalty(ExampleBasedNode first, ExampleBasedNode second) {
