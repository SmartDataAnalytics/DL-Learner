...
public class ParCELDoubleSplitterV1 implements ParCELDoubleSplitterAbstract {
	...
	public void init() throws ComponentInitException {
		...
		for (OWLDataProperty dp : reasoner.getDoubleDatatypeProperties())
			this.doubleDatatypeProperties.add(dp);
	...
	public Map<OWLDataProperty, List<Double>> computeSplits() {
		...
		Map<OWLDataProperty, ValuesSet> relations = new HashMap<>();

		OWLDataFactory factory = ontology.getOWLOntologyManager().getOWLDataFactory();

		for (OWLIndividual ind : positiveExamples) {
			Map<OWLDataProperty, ValuesSet> individualRelations = getInstanceValueRelation(ind, true, null);
		...
		for (OWLIndividual ind : negativeExamples) {
			Map<OWLDataProperty, ValuesSet> individualRelations = getInstanceValueRelation(ind, false, null);
	...
	private Map<OWLDataProperty, ValuesSet> getInstanceValueRelation(OWLIndividual individual,
		...
		Map<OWLDataPropertyExpression, Collection<OWLLiteral>> dataPropertyValues =  EntitySearcher.getDataPropertyValues(individual, ontology).asMap();

		// get all object properties value of the given individual
		Map<OWLObjectPropertyExpression, Collection<OWLIndividual>> objectPropertyValues = EntitySearcher.getObjectPropertyValues(individual, ontology).asMap();
		...
		for (OWLDataPropertyExpression dp : dataPropertyValues.keySet()) {
			...
			if (this.doubleDatatypeProperties.contains(dp)) {
				...
				// value
					...
					relations.put(dp.asOWLDataProperty(), values);
			}
		}
		...
		for (OWLObjectPropertyExpression op : objectPropertyValues.keySet()) {
			for (OWLIndividual ind : objectPropertyValues.get(op)) {
				Map<OWLDataProperty, ValuesSet> subRelations = getInstanceValueRelation(ind, positiveExample, visitedIndividuals);
