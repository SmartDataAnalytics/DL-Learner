...
/**
 * This class represents a value and its count (number of occurrence). This can be used to count the
 * occurrence of a value. In addition, each value may belong to a positive or negative example or to
 * both. <code>getType()</code> method return the type of the value. 0 for error (nether pos nor
 * neg), 1 for positive, 2 for negative, and 3 for either
 * 
 * @author An C. Tran
 */
...
public class ValueCount implements Comparable<ValueCount> {
	...
	public int compareTo(ValueCount v) {
		...
		return (this.value > v.value ? 1 : (this.value < v.value ? -1 : 0));
