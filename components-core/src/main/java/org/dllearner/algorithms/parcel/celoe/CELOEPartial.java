...
//	private OEHeuristicRuntime heuristicRuntime = new OEHeuristicRuntime();
	...
	private HashSet<OWLIndividual> unCoveredPos;
...
//		System.out.println("refined node: " + node);
	...
	private boolean addNode(OWLClassExpression description, OENode parentNode) {
		...
		if (accuracy >= 1) {
			...
			Set<OWLIndividual> coveredPosTmp = new HashSet<>();
...
//			System.out.println("minimum horizontal expansion is now " + minHorizExp);
	...
	public static class PartialDefinition {
		...
		double coverage = 0.0;
		List<Double> additionValue = new LinkedList<>();
		...
		private void initialAdditionalValue() {
			for (int i=0; i<this.maxAdditionalValue; i++)
				this.additionValue.add(0d);
		}

		public void setDescription(OWLClassExpression des) {
			this.description = des;
		}
		...
		public void setAdditionValue(int index, double v) {
			this.additionValue.set(index, v);
		}
		...
		/*
		public List<Double> getAdditionValue() {
