...
public class ParCELearnerMat extends ParCELAbstract implements ParCELearnerMBean {
	...
	@ConfigOption(defaultValue = "false", description = "This ask the learner continues exploring nodes in the search tree when all positive examples had been covered until all nodes reaches the length of the longest partial definition")
	...
	public void init() throws ComponentInitException {
		...
		if (refinementOperator == null) {
			...
			// -----------------------------------------
			Set<OWLClass> usedConcepts = new TreeSet<OWLClass>(reasoner.getClasses());
			...
			ClassHierarchy classHiearachy = (ClassHierarchy) reasoner.getClassHierarchy().cloneAndRestrict(
					usedConcepts);
	...
	*/
	 ...
	 */
	@Override
	public TreeSet<? extends EvaluatedDescription<ParCELScore>> getCurrentlyBestEvaluatedDescriptions() {
	 ...
	 */
	private TreeSet<? extends EvaluatedDescription<ParCELScore>> extraPLOENodesToEvaluatedDescriptions(
			Set<ParCELExtraNode> partialDefs) {
		TreeSet<EvaluatedDescription<ParCELScore>> result = new TreeSet<>();
		for (ParCELExtraNode node : partialDefs) {
			result.add(new EvaluatedDescription<ParCELScore>(node.getDescription(), new ParCELScore(node)));
		}
		return result;
	}
