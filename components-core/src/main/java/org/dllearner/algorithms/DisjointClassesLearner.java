...
public class DisjointClassesLearner extends AbstractAxiomLearningAlgorithm implements ClassExpressionLearningAlgorithm {
	...
	public void start() {
		...
		if(ks.isRemote()){
			...
			OntClass cls = ontModel.getOntClass(classToDescribe.toStringID());
			for(OntClass sub : cls.listSubClasses(false).toSet()){
				if(!sub.isAnon()){
					subClasses.add(df.getOWLClass(IRI.create(sub.getURI())));
				}
			}
			for(OntClass sup : cls.listSuperClasses().toSet()){
				if(!sup.isAnon()){
					subClasses.add(df.getOWLClass(IRI.create(sup.getURI())));
				}
			}
		}
...
//			runSPARQL1_1_Mode();
	...
	private void runSingleQueryMode(){
		...
		String query = String.format("SELECT ?type (COUNT(*) AS ?cnt) WHERE {?s a <%s>. ?s a ?type.} GROUP BY ?type", classToDescribe.toStringID());
	...
	private void runSPARQL1_0_Mode(){
		...
		String query = String.format(baseQuery, classToDescribe.toStringID(), classToDescribe.toStringID(), limit, offset);
		...
		while(!terminationCriteriaSatisfied() && newModel.size() != 0){
			...
			offset += limit;
			query = String.format(baseQuery, classToDescribe.toStringID(), classToDescribe.toStringID(), limit, offset);
	...
	public List<EvaluatedAxiom> getCurrentlyBestEvaluatedAxioms(int nrOfAxioms) {
		...
		for(EvaluatedDescription ed : getCurrentlyBestEvaluatedDescriptions(nrOfAxioms)){
			...
			axioms.add(new EvaluatedAxiom(df.getOWLDisjointClassesAxiom(descriptions), new AxiomScore(ed.getAccuracy())));
		}
		return axioms;
	}
	
	private List<EvaluatedDescription> buildEvaluatedClassDescriptions(Map<OWLClass, Integer> class2Count, Set<OWLClass> allClasses, int total){
		...
		for(OWLClass cls : completeDisjointclasses){
			if(useClassPopularity){
				...
				if(ks.isRemote()){
					...
					pop = ((LocalModelBasedSparqlEndpointKS)ks).getModel().getOntClass(cls.toStringID()).listInstances().toSet().size();
				}
			...
			if (useClassPopularity) {
				...
				if (ks.isRemote()) {
					...
					pop = ((LocalModelBasedSparqlEndpointKS) ks).getModel()
							.getOntClass(cls.toStringID()).listInstances().toSet()
							.size();
				}
				// we skip classes with no instances
				if (pop == 0)
...
//				System.out.println();
	...
	private void computeAllDisjointClassAxiomsOptimized(){
		...
		SortedSet<OWLClassExpression> superClasses = reasoner.getSuperClasses(classToDescribe);
	...
	public EvaluatedAxiom computeDisjointess(OWLClass clsA, OWLClass clsB){
		logger.debug("Computing disjointness between " + clsA + " and " + clsB + "...");
		
		//if clsA = clsB
		if(clsA.equals(clsB)){
			return new EvaluatedAxiom(df.getOWLDisjointClassesAxiom(clsA, clsB), new AxiomScore(0d, 1d));
		};
	...
	public Set<EvaluatedAxiom> computeDisjointness(Set<OWLClass> classes){
		Set<EvaluatedAxiom> axioms = new HashSet<EvaluatedAxiom>();
