...
@ComponentAnn(name="TCELOE", shortName="tceloe", version=1.0, description="CELOE is an adapted and extended version of the OCEL algorithm applied for the ontology engineering use case. See http://jens-lehmann.org/files/2011/celoe.pdf for reference.")
public class TCELOE2 extends AbstractCELA implements Cloneable{

	private static final Logger logger = LoggerFactory.getLogger(TCELOE2.class);
...
//	private OEHeuristicRuntime heuristicRuntime = new OEHeuristicRuntime();
	...
	private int initialT = 5000;

	@ConfigOption(defaultValue="1", description="TODO")  // TODO
	private int warmingFactor = 1;
	...
	private int t;
	private int numPickedRandomly;
	private int numPickedBest;
	private int numReturnedNodes;

	public TCELOE2() {}

	public TCELOE2(TCELOE2 celoe){
	...
	public TCELOE2(AbstractClassExpressionLearningProblem problem, AbstractReasonerComponent reasoner) {
...
//		logger.debug("Curr p: " + p);
		...
		if (random.nextDouble() < p) {
			//
			Set<OENode> nodes = searchTree.getNodeSet();

			int index = random.nextInt(nodes.size());
			Iterator<OENode> iter = nodes.iterator();
...
//			System.out.println("minimum horizontal expansion is now " + minHorizExp);
	...
	public void setInitialT(int initialT) {
		this.initialT = initialT;
	}

	public void setWarmingFactor(int warmingFactor) {
		this.warmingFactor = warmingFactor;
	}

	public int getWarmingFactor() {
		return warmingFactor;
	}
