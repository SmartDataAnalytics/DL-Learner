...
//	private OEHeuristicRuntime heuristicRuntime = new OEHeuristicRuntime();
	...
	public CELOENiching(CELOENiching celoe){
	...
	public CELOENiching(AbstractClassExpressionLearningProblem problem, AbstractReasonerComponent reasoner) {
	...
	private OENode getNextNodeToExpand() {
		...
		while(it.hasNext()) {
			...
			if (isExpandAccuracy100Nodes() &&
					...
					(searchTree.size() < 2 * Math.ceil(1/similarityPenaltyThreshold) ||similarityPenalty < similarityPenaltyThreshold)) {

					return node;
			} else {
				if((node.getAccuracy() < 1.0 || node.getHorizontalExpansion() < OWLClassExpressionUtils.getLength(node.getDescription())) &&
						(searchTree.size() < 2 * Math.ceil(1/similarityPenaltyThreshold) ||
...
//		lp.setEquivalence(false);
	...
	protected double computeSimilarityPenalty(OENode first, OENode second) {
