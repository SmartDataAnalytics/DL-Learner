	...
	public void init() throws ComponentInitException {
		...
		*/
		...
		if (operator instanceof CustomHierarchyRefinementOperator) {
			...
			((CustomHierarchyRefinementOperator) operator).setObjectPropertyHierarchy(reasoner
					.getObjectPropertyHierarchy());
			((CustomHierarchyRefinementOperator) operator).setDataPropertyHierarchy(reasoner
		...
		if (isClassLearningProblem) {
			...
			if (isEquivalenceProblem) {
				...
				if (reuseExistingDescription && (existingDefinitions.size() > 0)) {
					...
					LengthLimitedRefinementOperator upwardOperator = (LengthLimitedRefinementOperator) new OperatorInverter(
							operator);
					...
					if (startClass.equals(existingDefinition)) {
						logger.info("Reusing existing description {} as start class for learning algorithm.",
								startClass.toManchesterSyntaxString(baseURI, prefixes));
					} else {
						logger.info(
								"Generalised existing description {} to {}, which is used as start class for the learning algorithm.",
								existingDefinition.toManchesterSyntaxString(baseURI, prefixes),
								startClass.toManchesterSyntaxString(baseURI, prefixes));
					}
			...
			examples = Helper.union(((PosNegLP) learningProblem).getPositiveExamples(),
...
//		System.out.println("isRunning: " + isRunning);
	...
	private boolean addNode(Description description, OENode parentNode) {
		...
		if (accuracy > 1.0 || (accuracy < 0.0 && accuracy != -1)) {
			logger.warn(
					"Invalid accuracy value {} for description {}. This could be caused by a bug in the heuristic measure and should be reported to the DL-Learner bug tracker.",
		...
		if (singleSuggestionMode) {
			if (accuracy > bestAccuracy) {
				...
				logger.info("more accurate (" + dfPercent.format(bestAccuracy) + ") class expression found: "
						+ descriptionToString(bestDescription)); // + getTemporaryString(bestDescription)); 
			}
			return true;
		}
		...
		if (!isCandidate) {
			...
			isCandidate = (accuracy > accThreshold || (accuracy >= accThreshold && description.getLength() < worst
	...
	private boolean isDescriptionAllowed(Description description, OENode parentNode) {
		...
		if (parentNode != null
				&& ConceptTransformation.getForallOccurences(description) > ConceptTransformation
						.getForallOccurences(parentNode.getDescription())) {
			...
			SortedSet<PropertyContext> parentContexts = ConceptTransformation.getForallContexts(parentNode
					.getDescription());
	...
	private boolean terminationCriteriaSatisfied() {
		return stop
				|| (maxClassExpressionTestsAfterImprovement != 0 && (expressionTests
						- expressionTestCountLastImprovement >= maxClassExpressionTestsAfterImprovement))
	...
	private String bestDescriptionToString() {
		...
		return best.getDescription().toManchesterSyntaxString(baseURI, prefixes) + " (accuracy: "
				+ dfPercent.format(best.getAccuracy()) + ")";
	}
	...
	public void setMaxClassExpressionTestsWithoutImprovement(int maxClassExpressionTestsWithoutImprovement) {
	...
	public static void main(String[] args) throws Exception {
		...
		Query query = QueryFactory.create("SELECT (COUNT(distinct ?s) as ?cnt)  WHERE {" + "?s a <" + cls + ">.}",
				Syntax.syntaxARQ);
		...
		query = QueryFactory.create("SELECT ?p (COUNT(distinct ?s) AS ?cnt) WHERE {"
				+ "?s ?p ?o. ?p a <http://www.w3.org/2002/07/owl#ObjectProperty>." + "?s a <" + cls + ">. " +
				//				"OPTIONAL{?x ?p ?o1. " +
