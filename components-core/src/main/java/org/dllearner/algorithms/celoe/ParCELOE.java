...
//	}
	...
	public ParCELOE(ParCELOE celoe){
	...
	public ParCELOE(AbstractLearningProblem problem, AbstractReasonerComponent reasoner) {
...
//		System.out.println(configurator.getMaxExecutionTimeInSeconds());
		...
		int minNumberOfWorker = 2;
		int maxNumberOfWorker = 4;
		long keepAliveTime = 1000;
		int maxTaskQueueLength = 2000;
		
		refinementOperatorPool = new StackObjectPool<RhoDRDown>(new RefinementOperatorFactory(reasoner), maxNumberOfWorker + 1);
		
		
		BlockingQueue<Runnable> taskQueue = new LinkedBlockingQueue<Runnable>(maxTaskQueueLength);
		...
		while (!terminationCriteriaSatisfied()) {
			...
			if(nextNode != null && !workerPool.isShutdown() && !workerPool.isTerminating()) {
				// apply operator
				try {
					workerPool.execute(new CELOEWorker(nextNode));
...
//		System.out.println("isRunning: " + isRunning);
	...
	private synchronized TreeSet<Description> refineNode(OENode node) {
...
//		System.out.println(refinements);
	...
	private synchronized boolean addNodeToSearchTree(Description description, OENode parentNode) {
...
//	}
	...
	private synchronized void updateMinMaxHorizExp(OENode node) {
...
//			System.out.println("minimum horizontal expansion is now " + minHorizExp);
	...
	class CELOEWorker implements Runnable {
		
		private OENode node;

		public CELOEWorker(OENode node) {
			this.node = node;
		}
		...
		public void run() {
			...
			TreeSet<Description> refinements = refineNode(node);
...
//					System.out.println("potentially adding " + refinement + " to search tree as child of " + nextNode + " " + new Date());
					...
					addNodeToSearchTree(refinement, node);
					mon2.stop();
...
//				System.out.println("  refinement queue length: " + refinements.size());
		...
		private TreeSet<Description> refineNode(OENode node) {
			...
			RhoDRDown operator;
			try {
				operator = refinementOperatorPool.borrowObject();
				TreeSet<Description> refinements = (TreeSet<Description>) operator.refine(node.getDescription(), horizExp+1);
				refinementOperatorPool.returnObject(operator);
				node.incHorizontalExpansion();
				node.setRefinementCount(refinements.size());
				searchTree.add(node);
				return refinements;
	...
	class RefinementOperatorFactory extends BasePoolableObjectFactory<RhoDRDown> {
		
		private AbstractReasonerComponent reasoner;

		public RefinementOperatorFactory(AbstractReasonerComponent reasoner) {
		 ...
		 */
		@Override
		public RhoDRDown makeObject() throws Exception {
			RhoDRDown op = new RhoDRDown();
			op.setReasoner(reasoner);
			op.setClassHierarchy(reasoner.getClassHierarchy().clone());
			op.init();
			return op;
		}
	}
...
//		org.apache.log4j.Logger.getLogger(PCELOE.class).setLevel(Level.DEBUG);
		...
		ParCELOE alg = new ParCELOE(lp, rc);
