...
public class ParCELearnerExV1 extends ParCELExAbstract implements ParCELearnerMBean {	
		...
		if (logger.isInfoEnabled()) {			
			synchronized (partialDefinitions) {
				...
				for (ParCELExtraNode def : compactedDefinitions) {
					logger.info(count++ + ". " + ParCELExUtilities.groupDefinition(def.getDescription()).toString().replace("and (not", "\nand (not") + //def.getDescription().toManchesterSyntaxString(baseURI, prefix) +
	...
	private void reset() {
		...
		this.potentialPartialDefinitions = new TreeSet<ParCELExtraNode>(new ParCELCompletenessComparator());
	 ...
	 */
	private TreeSet<? extends EvaluatedDescription<? extends Score>> extraPLOENodesToEvaluatedDescriptions(Set<ParCELExtraNode> partialDefs) {
