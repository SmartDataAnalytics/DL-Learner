 ...
 *
 */
public class ParCELWorkerExV12 implements Runnable {
	...
	private Logger logger = Logger.getLogger(ParCELWorkerExV12.class);
	 ...
	 * @param refinementOperatorPool Refinement operator used to refine the given node
	 ...
	 * @param name Name of the worker, assigned by reduce (for tracing purpose only)
	 */
	public ParCELWorkerExV12(ParCELearnerExV1 reducer, LengthLimitedRefinementOperator refinementOperator,
	...
	public void run() {
		...
		HashSet<ParCELExtraNode> newPotentialPartialDefinitions = new HashSet<ParCELExtraNode>();
		...
		if (refinements != null) {
			...
			while (refinements.size() > 0) {
				...
				int refinementLength = OWLClassExpressionUtils.getLength(refinement);
	...
	private TreeSet<OWLClassExpression> refineNode(ParCELNode node) {
		...
		try {
			refinements = (TreeSet<OWLClassExpression>) refinementOperator.refine(node.getDescription(), horizExp+1);
