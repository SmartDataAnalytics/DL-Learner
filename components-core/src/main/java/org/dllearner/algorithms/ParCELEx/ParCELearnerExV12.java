...
public class ParCELearnerExV12 extends ParCELExAbstract implements ParCELearnerMBean {	
	...
	public void init() throws ComponentInitException {
		...
		if (refinementOperator == null) {
			...
			//-----------------------------------------
			Set<OWLClassExpression> usedConcepts = new TreeSet<>(reasoner.getClasses());
		...
		if (logger.isInfoEnabled()) {
			synchronized (partialDefinitions) {
				if (this.getCurrentlyOveralMaxCompleteness() == 1)
					logger.info(
							"Learning finishes in: " + this.miliLearningTime + "ms, with: " + partialDefinitions.size() + " definitions");
				else if (this.isTimeout()) {
					logger.info(
							"Learning timeout in " + this.maxExecutionTimeInSeconds + "ms. Overall completeness (%): " + this.getCurrentlyOveralMaxCompleteness());
					logger.info(
							"Uncovered positive examples left " + this.uncoveredPositiveExamples.size() + " - " + ParCELStringUtilities.replaceString(
									this.uncoveredPositiveExamples.toString(), this.baseURI, this.prefix));
				} else {
					logger.info(
							"Learning is manually terminated at " + this.miliLearningTime + "ms. Overall completeness (%): " + this.getCurrentlyOveralMaxCompleteness());
					logger.info(
							"Uncovered positive examples left " + this.uncoveredPositiveExamples.size() + " - " + ParCELStringUtilities.replaceString(
									this.uncoveredPositiveExamples.toString(), this.baseURI, this.prefix));
				}
				...
				for (ParCELExtraNode def : compactedDefinitions) {
					logger.info(
							count++ + ". " + ParCELExUtilities.groupDefinition(def.getDescription()).toString().replace(
									"and (not",
									"\nand (not") + //def.getDescription().toManchesterSyntaxString(baseURI, prefix) +
									" (length:" + OWLClassExpressionUtils.getLength(def.getDescription()) +
					...
					if (logger.isDebugEnabled()) {
						...
						for (OENode n : processingNodes) {
							...
							while (parent != null) {
								while (parent != null) {
									logger.debug("  <-- " + parent.getDescription().toString());
									...
									for (OENode child : children) {
										...
										logger.debug("    --> " + tmp.getDescription().toString());
		...
		for (ParCELExtraNode def : definitions) {
			...
			if (uncoveredPositiveExamplesRemoved > 0) {
				...
				if (logger.isTraceEnabled()) {
					logger.trace("PARTIAL definition found: " + def.getDescription().toString() +
					...
					logger.debug("PARTIAL definition found: " + def.getDescription().toString() +
										 "\n\t - covered positive examples (" + def.getCoveredPositiveExamples().size() + "): " +def.getCoveredPositiveExamples() +
										 "\n\t - uncovered positive examples left: " + uncoveredPositiveExamplesSize + "/" + positiveExamples.size()
					);
		...
		for (ParCELExtraNode def :  potentialPartialDefinitions) {
			if (this.potentialPartialDefinitions.add(def.getDescription())) {
				...
				if (logger.isInfoEnabled())
					...
					logger.debug("POTENTIAL PARTIAL DEFINITION found. " + def.getDescription().toString() +
							"Uncovered positive examples left: " + uncoveredPositiveExamplesSize + "/" + positiveExamples.size());
	...
	public void newCounterPartialDefinitionsFound(Set<ParCELExtraNode> counterPartialDefinitions) {
		...
		for (ParCELExtraNode def : counterPartialDefinitions) {
			...
			if (numberOfNewCoveredNegativeExamples > 0) {
				...
				if (logger.isTraceEnabled()) {
					logger.trace("COUNTER PARTIAL definition found: " + def.getDescription().toString() +
					...
					logger.debug("COUNTER PARTIAL definition found: " + def.getDescription().toString() +
							"\n\t - covered negative examples (" + def.getCoveredNegativeExamples().size() + "): " + def.getCoveredNegativeExamples() +
							"\n\t - total covered negative examples: " + numberOfCoveredNegativeExamples + "/" + this.negativeExamples.size() 
							);
				else if (logger.isInfoEnabled()) {
	...
	private void reset() {
		...
		this.coveredNegativeExamples = new HashSet<OWLIndividual>();		
