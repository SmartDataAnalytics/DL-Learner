...
public class Configuration {
	...
	public static Configuration getConfiguration(URI uri){
		//public static String getTellsString(URL file,  URI kbURI){//throws OWLOntologyCreationException{
			...
			while (it.hasNext()){
				...
				if(tmp.getURI().equals(uri)){
					...
					arr=getIndividualsForProperty("hasTypedQuery",tmp.getObjectPropertyValues(ontology));
		...
		public static OWLIndividual[] getIndividualsForProperty(String propertyname,Map<OWLObjectPropertyExpression, Set<OWLIndividual>> m){
		...
		public static String getFirstValueForDataProperty(String propertyname,Map<OWLDataPropertyExpression, Set<OWLConstant>> m){
			return getValuesForDataProperty(propertyname, m)[0];
		}
				
		public static String[] getValuesForDataProperty(String propertyname,Map<OWLDataPropertyExpression, Set<OWLConstant>> m){
		...
		public static SparqlEndpoint makeEndpoint(OWLIndividual sEndpoint,OWLOntology o){
			...
			String hasAfterGET=getFirstValueForDataProperty("hasAfterGET",sEndpoint.getDataPropertyValues(o));
			String hasQueryParameter=getFirstValueForDataProperty("hasQueryParameter",sEndpoint.getDataPropertyValues(o));
			OWLIndividual[] para=getIndividualsForProperty("hasGETParameter",sEndpoint.getObjectPropertyValues(o));
			//System.out.println("test");
			HashMap<String,String> parameters=new HashMap<String,String>();
			if(para==null)return new SparqlEndpoint( host,  port,  hasAfterGET,  hasQueryParameter,  parameters);
			for (OWLIndividual p : para) {
				//System.out.println("test2");
				String a1=getFirstValueForDataProperty("hasParameterName",p.getDataPropertyValues(o));
				String a2=getFirstValueForDataProperty("hasParameterContent",p.getDataPropertyValues(o));
				parameters.put(a1, a2);
			}
			//System.out.println("test2");
			//System.out.println(host+port+ hasAfterGET+  hasQueryParameter+  parameters);
		...
		public static SparqlQueryType makeSparqlQueryType(OWLIndividual typedQuery,OWLOntology o){
			String useLiterals=getFirstValueForDataProperty("usesLiterals",typedQuery.getDataPropertyValues(o));
			String hasMode=getFirstValueForDataProperty("hasMode",typedQuery.getDataPropertyValues(o));
			//String hasAfterGET=getValuesForDataProperty("hasAfterGET",sEndpoint.getDataPropertyValues(o));
			//String hasQueryParameter=getValuesForDataProperty("hasQueryParameter",sEndpoint.getDataPropertyValues(o));
			OWLIndividual[] objFilter=getIndividualsForProperty("hasObjectFilterSet",typedQuery.getObjectPropertyValues(o));
			OWLIndividual[] predFilter=getIndividualsForProperty("hasPredicateFilterSet",typedQuery.getObjectPropertyValues(o));
