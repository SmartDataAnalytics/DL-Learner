...
<p><input type="submit" value="Form Call with Callback Object" onclick="var cb = setupCallback(); xajax.call('testForm', { parameters:[xajax.getFormValues('testForm1'), true], callback: cb }); return false;" /></p>
<p><input type="submit" value="Form Call with Overridden Callback" onclick="var cb = setupCallback(); xajax.call('testForm', { parameters:[xajax.getFormValues('testForm1'), true], callback: cb, onRequest: function() { alert('Overridden onRequest'); } }); return false;" /></p>
<p><input type="submit" value="Form Call with Inline Callback (also onFailure test)" onclick="var cb = setupCallback(); xajax.call('testFormFail', { parameters:[xajax.getFormValues('testForm1'), true], onRequest: function() { alert('In inline onRequest'); }, onFailure: function(args) { alert('In inline onFailure -- status is: ' + args.request.status); } }); return false;" /></p>
<p><input type="submit" value="Test onExpiration" onclick="xjx.$('waiting').style.visibility = 'visible'; xajax.call('testFormExpire', { parameters:[xajax.getFormValues('testForm1'), true], onExpiration: function(args) { alert('In inline onExpiration'); xajax.abortRequest(args.request, args.options) }, onComplete: function() { xjx.$('waiting').style.visibility = 'hidden'; } }); return false;" /> <span id="waiting" style="visibility: hidden">waiting...</span></p>
