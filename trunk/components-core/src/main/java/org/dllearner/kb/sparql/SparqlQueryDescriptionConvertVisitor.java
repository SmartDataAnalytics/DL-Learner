...
public class SparqlQueryDescriptionConvertVisitor implements DescriptionVisitor {
    ...
    public String getSparqlQuery(Description description) {
        ...
        if (count) {
            ...
            ret = "SELECT " + distinct() + "?subject " + ((labels) ? "?label" : "") + " { "
                    + labels() + query + " \n" + customFilterTmp + "\n } " + limit();
        }
        reset();
        return ret;
    ...
    public static String getSparqlQueryIncludingSubclasses(String descriptionKBSyntax,
            int resultLimit, SPARQLTasks st, int maxDepth) throws ParseException {
    ...
    public static void testHasValue() throws Exception {
        // String ttt =
        // "(\"http://dbpedia.org/ontology/Plant\" AND (\"http://dbpedia.org/ontology/kingdom\" hasValue \"http://dbpedia.org/resource/Plantae\" OR EXISTS \"http://dbpedia.org/ontology/family\".\"http://dbpedia.org/ontology/FloweringPlant\"))";
    ...
    public static void testTrans() throws Exception {
        ...
        SparqlQueryDescriptionConvertVisitor testVisitor = new SparqlQueryDescriptionConvertVisitor();
        testVisitor.setTransitiveProperties(new TreeSet<String>(Arrays
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            subclassMap.put("http://nlp2rdf.org/ontology/Sentence",
                    "<http://nlp2rdf.org/ontology/Subsentence>");
            ...
            testMap.put(prefix + "Sentence",
                    new HashSet<String>(Arrays.asList(new String[] { "whatever", "loser" })));
    ...
    public void visit(ObjectAllRestriction description) {
        ...
        customFilter += "\n FILTERS NOT EXISTS { ?object a ?object" + currentObject++
                + " FILTER (?object" + currentObject + " != <"
                + description.getChild(0) + "> ) }";
        currentObject++;
    }
    ...
    public void visit(ObjectSomeRestriction description) {
        ...
        if (transitiveProperties != null
                && transitiveProperties.contains(description.getRole().toString())) {
            option = " OPTION (TRANSITIVE , t_in(?" + stack.peek() + "), t_out(?object"
                    + currentObject + "), T_MIN(0), T_MAX(6), T_DIRECTION 1 , T_NO_CYCLES) ";
        }
        ...
        if (description.getChild(0) instanceof Thing) {
            ...
            query += "\n?" + stack.peek() + " <" + description.getRole() + "> ?object"
    ...
    public void visit(DatatypeValueRestriction description) {
        ...
        query += "\n?" + stack.peek() + " <" + description.getRestrictedPropertyExpression()
                + ">  \"" + description.getValue().getLiteral() + "\" ";
    }
