...
@ComponentAnn(name = "PDLLPosNegLP", shortName = "pdllPosNegLP", version = 0.1, description = "PDLL positive negative learning problem")
public class PADCELPosNegLP extends AbstractLearningProblem {
	 ...
	 *            Negative examples
	 */
	public PADCELPosNegLP(AbstractReasonerComponent reasoningService,
	 ...
	 */
	public PADCELPosNegLP() {
		super();
		
		if (logger.isDebugEnabled())
			logger.debug("Learning problem created: " + this);
	}
	...
	/**
	 * Calculate accuracy and correctness, calculated by:<br>
	 ...
	 * 
	 * @return A PDLLEvaluationResult object. If the description is weak, its accuracy will be -1
	 * 
	 *         NOTE: do we need "weak" concept with the value of -1? How if we just simply assign 0
	 *         for it?
	 */
	public PADCELEvaluationResult getAccuracyAndCorrectness(Description description) {
		...
		if (coveredPositiveExamples.size() > 0) {
			...
			return new PADCELEvaluationResult(accuracy, correctness, completeness,
			...
			return new PADCELEvaluationResult(-1, notCoveredNeg / (double) negativeExamples.size(), 0);
		}
	 ...
	 * @return
	 */
	public PADCELEvaluationResult getAccuracyAndCorrectness2(Description description) {
		...
		if (noOfUpdatedCoveredPositiveExamples > 0) {
			...
			// a node will be considered as "weak" if it covers none of the positive example and
			// the accuracy will be assigned -1
			return new PADCELEvaluationResult(-1, 0, 0);
		}
	 ...
	 * @return
	 */
	public PADCELEvaluationResult getAccuracyAndCorrectness3(Description description) {
	 ...
	 * @return
	 */
	public PADCELEvaluationResult getAccuracyAndCorrectnessEx(Description description) {
		...
		PADCELEvaluationResult result = new PADCELEvaluationResult();
	 ...
	 */
	@Override
	public PADCELScore computeScore(Description description) {
		double correctness = this.correctness_cal(description);
		double accuracy = this.accuracy_cal(description);

		return new PADCELScore(accuracy, correctness);
	}
	...
	public EvaluatedDescription evaluate(Description description) {
		PADCELScore score = this.computeScore(description);
