...
public class SparqlQueryConventional extends SparqlQueryAbstract{
	...
	private String sendAndReceiveSPARQL(String sparql) throws IOException {
		...
		//SpecificSparqlEndpoint specificSparqlEndpoint = configuration.getSparqlEndpoint();
		p("URL: "+specificSparqlEndpoint.getURL());
		p("Host: "+specificSparqlEndpoint.getHost());
		
		connection = (HttpURLConnection) specificSparqlEndpoint.getURL().openConnection();
		connection.setDoOutput(true);

		//connection.addRequestProperty("Host", specificSparqlEndpoint.getHost());
		...
		while (it.hasNext()) {
			...
			FullURI += "" + URLEncoder.encode(element, "UTF-8") + "="
					+ URLEncoder.encode(specificSparqlEndpoint.getParameters().get(element), "UTF-8") + "&";
		}
		
		FullURI += "" + specificSparqlEndpoint.getHasQueryParameter() + "=" + URLEncoder.encode(sparql, "UTF-8");
		p(FullURI);
		osw.write(FullURI);
		osw.close();
